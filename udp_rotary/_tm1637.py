...
TM1637_DELAY = 0.0000000001  # 10us delay between clk/dio pulses
...
class TM1637(object):
    ...
    def __init__(self, clk, dio, brightness=7):
        self.clk = mraa.Gpio(clk)
        self.dio = mraa.Gpio(dio)
    ...
    def dec_temperature(self, num):
        if num < -9.9:  # limit to single digit negatives
            ...
            if num == 0:  # exact zero
                ...
                try:
                    # will fail if there is no decimal point
                    seg3 = self.encode_digit(int(str(num).split('.')[1][0]))
                except:
                    seg3 = self.encode_digit(0)
            # segments and degree character
